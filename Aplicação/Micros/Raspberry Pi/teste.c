...
int main()
{
    ...
    uint8_t decryptedtext[BC_len];
    uint8_t ciphertext[BC_len];
    uint8_t* mensagem= "012345678901234";
   
  
  // criptografando mensagem
	
	if(bc_aes_cbc_enc(ciphertext,&outputsize,mensagem,BC_len,key,BC_len*2,iv)){
	...
	if(bc_aes_cbc_dec(decryptedtext,&out_len,ciphertext,BC_len,key,BC_len,iv)){
